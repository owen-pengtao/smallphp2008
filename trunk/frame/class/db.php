...
class db {
	...
	private function sql_update($tab, $row, $where){
		foreach ($row as $key=>$value) {
			$sqlud .= $key."= '".$value."',";
		}
		return "UPDATE `".$tab."` SET ".substr($sqlud, 0, -1)." WHERE ".$where;
	}
	private function sql_delete($tab, $where){
	 ...
	 * @author owen 2008-6-10
	 */
	private function _halt($error_str=""){
	 ...
	 * @author owen 2008-6-10
	 */
	private function _free_result($rs){
		@mysql_free_result($rs);
	}
	/**
	 * 从结果集中取得一行作为关联数组
	 ...
	 * @author owen 2008-6-10
	 */
    private function _fetch_array($sql=''){
