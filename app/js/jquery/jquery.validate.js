...
jQuery.extend(jQuery.validator, {
	...
	messages: {
		...
		url: "请输入正确的 URL 地址。",
		date: "请输入正确的日期格式， 2008-08-08。",
		dateISO: "请输入正确的日期(ISO)格式， 2008/08/08,2008-08-08。",
		...
		equalTo: "请保持一致的输入。",
		accept: "文件类型不支持。",
		maxLength: jQuery.format("字符长度过长，M个以内。"),
		minLength: jQuery.format("字符长度过短，至少N个。"),
		rangeLength: jQuery.format("字符长度在(N-M)之间。"),
		rangeValue: jQuery.format("数字范围在(N-M)之间。"),
		maxValue: jQuery.format("数字必须小于等于M。"),
		minValue: jQuery.format("数字必须大于等于N。")
	},
	...
	methods: {
		...
		email: function(value, element) {
			...
			return this.required(element) || /^[A-Z0-9._%-]+@[A-Z0-9._%-]+\.[A-Z]{2,6}$/i.test(value);
		},
